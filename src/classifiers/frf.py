...
class FairDecisionTreeClassifier():
    ...
    def fit(self, X="X", y="y", b="bias"):
        ...
        def get_best_split(indexs):
            ...
            return best_score, best_feature, best_split_value #type: ignore
    ...
    def predict(self, X):
        ...
        probas = predict_proba(X)[:,1] #type: ignore
...
class FairRandomForestClassifier():
    ...
    def fit(self, X, y, s, verbose=False):
        ...
        fit_dts = [tree for fit_dt_batch in fit_dt_batches for tree in fit_dt_batch] #type: ignore
        self.trees = fit_dts
        self.fit = True #type: ignore
    ...
    def predict_proba(self, X):
        ...
        if not self.fit:
            ...
            y_prob = y_preds[0] #type: ignore
            for i in range(1, len(y_preds)): #type: ignore
                y_prob = np.concatenate(
                    (y_prob, y_preds[i]), #type: ignore
                    axis=0
                )
    ...
    def predict(self, X):
        ...
        if not self.fit:
            ...
            predictions = mode(
                np.concatenate((
                    y_preds,
                    np.repeat(self.y_pos, y_preds.shape[1]).reshape(1, y_preds.shape[1]) #type: ignore
                ), axis=0))[0][0]
            return predictions
...
def classify(Xys, folds, theta, max_depth, n_bins, outer_fold, inner_fold=None):
    ...
    if not os.path.isfile(filepath + '.pkl'):
        ...
        clf.fit_time = fit_time #type: ignore
        joblib.dump(clf, filepath + '.pkl')
    else:
        clf = joblib.load(filepath + '.pkl')
        fit_time = clf.fit_time
    probs = clf.predict_proba(X_test)[:,1] #type: ignore             
